...
client.connect({}, (frame) => {
  client.subscribe("/topic/messages", (payload) => {
    ...
    var recipientId = chatMessage.recipientId; 
    var content = chatMessage.content; 

    const chatRoomProfile = document.getElementById(chatRoomId);
    const timeElement = chatRoomProfile.querySelector(".time");
    const previewElement = chatRoomProfile.querySelector(".preview");
    ...
    if(currentUserId != senderId){
      var chatRoomElement = document.querySelector("#chat-" + chatRoomId);
      var bubbleHtml = '<div class="' + "bubble you" + '">' + content + "</div>";
      chatRoomElement.insertAdjacentHTML("beforeend", bubbleHtml);
    }
  });
});
